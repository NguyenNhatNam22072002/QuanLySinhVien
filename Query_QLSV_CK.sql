...
exec sp_addrolemember 'db_Admin1' , 'Admin1'
...
grant select on Khoa to db_GiangVien1
grant select on HeDT to db_GiangVien1
grant select on KhoaHoc to db_GiangVien1
grant select on Lop to db_GiangVien1
exec sp_addrolemember 'db_GiangVien1','GiangVien1'

CREATE PROC DiemOfSV
AS
BEGIN
	SELECT * FROM SinhVien_Diem
END
GO

CREATE PROC GiangVienQLDiem
AS
BEGIN
	SELECT * FROM GiangVien_QuanLyDiem
END
GO
