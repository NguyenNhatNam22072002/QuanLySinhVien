	...
	--XoaSinhVien '20110347'
	--XoaDiemSinhVien '20110347'
	--select * from Diem
	...
	--ThemMoiSinhVien '20110523', N'Nguyễn Nhật Nam' , 1, '2002/12/28', N'Bình Định', '0898463214', 'IT1'
	...
	--ThemDiemSinhVien '21136777','MATH132401', '2',null, '8', '9'
	--select * from dbo.Diem
	--select * from SinhVien
	...
	Select * from SinhVien
	select * from Diem
	XoaSinhVien '20110001'
	XoaDiemSinhVien '20110001'
	-- kiểm tra --
	...
	go
	-- kiểm tra --
	--ThemKhoa 'OTO2',N'Công nghệ ô tô', N'Tầng 3- Khu B', '03474944216'
	--select * from Khoa
	...
	go
	---- kiểm tra --
	--SuaKhoa 'OTO',N'Công nghệ ô tô', N'Tầng 3 - Khu E', '0379524871'
	--SuaKhoa 'OTO2',N'Công nghệ ô tô', N'Tầng 4- Khu B', '0396714823'
	--select * from Khoa
	...
	as 
	begin 
		select MaSV, TenSV, case GioiTinh when 0 then N'Nữ' when 1 then N'Nam' end as 'GioiTinh', NgaySinh, QueQuan, SoDienThoai from SinhVien 
	end
	go
	--Show_DSSinhVien
	--drop proc Show_DSSinhVien
	...
	gos
	-- kiểm tra --
	--SuaThongTinMonHoc 'STMA240121', N'Sức Bền Vật Liệu', '4'
	--select * from MonHoc
	...
	create trigger trigger_insert_updateDiem
	...
	drop trigger trigger_insertTuoi_SinhVien
	---->DONE
	--select * from SinhVien
	--select * from Diem
	...
	create trigger trigger_insert_updateTuoi_SinhVien on SinhVien for insert, update
	as
	begin
	declare @count int = 0
	...
	go
	--->DONE
	--ThemMoiSinhVien '20110333', N'Cao Huỳnh Bảo An' , 1, '2001/12/28', N'Bình Định', '0898463214', 'IT1'
	--UpdateSinhVien '20110333', N'Cao Huỳnh Bảo An' , 1, '2003/12/28', N'Bình Định', '0898463214', 'IT1'
	--XoaSinhVien '20110333'
	--select * from SinhVien

--trigger không được phép bỏ trống thông tin sinh viên --
...
-----> DONE
	...
	go
	---> DONE
	--select * from Show_ThongTinSinhVien(20110001)
	--drop function Show_ThongTinSinhVien
	--select * from SinhVien
	--select * from Diem
	--select * from MonHoc
	-- xem điểm sinh viên phân quyền sinh viên --
	create function ShowDiemSinhVien(@MaSV char(10))
	returns table as
	return(
		select dbo.Diem.MaSV, TenSV, MaMH, HocKy, DiemQuaTrinhLan1, DiemQuaTrinhLan2, DiemCuoiKi from dbo.SinhVien, dbo.Diem
		where SinhVien.MaSV = Diem.MaSV and @MaSV = Diem.MaSV
	)
	select * from ShowDiemSinhVien(20110001)
	select * from Diem
	drop table Diem

	drop function ShowDiemSinhVien
	...
	-- function trả về số lượng sinh viên trong 1 môn học --
	create function SoLuong_SinhVienCuaMonHoc(@MaMH nvarchar(15))
	returns int
	as
	begin
		declare @count int
		select @count = count(Diem.MaMH) from Diem where @MaMH = Diem.MaMH
		return @count
	end
	go
	---> DONE
	--select dbo.SoLuong_SinhVienCuaMonHoc('MATH132401')
	--drop function SoLuong_SinhVienCuaMonHoc
	--select * from Diem

	-- function tìm kiếm sinh viên bảng sinh viên ---
	create function TimKiemSV(@TenSV nvarchar(50))
	returns table as
	return(
		select * from dbo.SinhVien where TenSV like '%' + @TenSV + '%'
	)
	drop function TimKiemSV
	select * from dbo.TimKiemSV(N'Nguyễn %')

	-- function tìm kiếm điểm sinh viên --
	create function TimDiemSV(@MaSV nvarchar(20))
	returns table as
	return
		select dbo.SinhVien.TenSV, dbo.Diem.MaSV, dbo.MonHoc.MaMH, dbo.MonHoc.TenMH, HocKy, DiemQuaTrinhLan1, DiemQuaTrinhLan2, DiemCuoiKi from dbo.Diem, dbo.SinhVien, dbo.MonHoc 
		where Diem.MaSV like'%' + @MaSV + '%' and SinhVien.MaSV = Diem.MaSV and Diem.MaMH = MonHoc.MaMH and @MaSV = Diem.MaSV

	select * from GiangVien
	select * from Khoa
	select * from Lop

	-- function tìm kiếm giảng viên --
	create function TimKiemGV(@MaGV nvarchar(20))
	returns table as
	return(
			select * from dbo.GiangVien where MaGV like '%' + @MaGV + '%'
	)
	select * from TimKiemGV('GV6')

	-- fucntion tìm kiếm môn học --
	create function TimKiemMonHoc(@MaMH nvarchar(20))
	returns table as
	return(
		select * from MonHoc where MaMH like '%' + @MaMH + '%'
	)
	select * from TimKiemMonHoc('MPRO232103')
	select * from MonHoc
	...
	select * from TinhDiemTongKet
	select * from SinhVien_Khoa

create view SV_Show as
select MaSV, TenSV, case GioiTinh when 0 then N'Nữ' when 1 then N'Nam' end as 'GioiTinh', NgaySinh, QueQuan, SoDienThoai 
from SinhVien
